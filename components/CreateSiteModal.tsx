...
interface CreateSiteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (newSiteData: Omit<Website, 'id' | 'thumbnailUrl'>) => void;
}
...
const CreateSiteModal: React.FC<CreateSiteModalProps> = ({ isOpen, onClose, onCreate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60" aria-modal="true" role="dialog" onClick={onClose}>
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md m-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white">새 웹사이트 만들기</h3>
          <button
            onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                placeholder="웹사이트에 대한 간략한 설명입니다."
                required
              ></textarea>
            </div>
            <div>
